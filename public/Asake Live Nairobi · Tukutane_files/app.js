...
`);
  ...
  Hooks2.Countdown = {
    mounted() {
      var countDownDate = (/* @__PURE__ */ new Date("Dec 20, 2025 14:00:00")).getTime();
  ...
  Hooks2.Pdf = {
    // The mounted() callback is the ideal place for one-time setup.
    mounted() {
      const ticketElement = this.el;
      const ticketIdElement = ticketElement.querySelector("#ticketid");
      const downloadButton = document.getElementById("print_small_ticket");
      if (!ticketIdElement || !downloadButton) {
        console.error(
          "PDF Hook Error: Could not find the ticket ID or download button."
        );
        return;
      }
      const ticketId = ticketIdElement.innerText.trim();
      const downloadTicket = () => {
        ...
        const options = {
          callback: (doc3) => {
            const fileName = `Asake-Live-Ticket-${ticketId}.pdf`;
            doc3.save(fileName);
            downloadButton.innerText = originalButtonText;
            downloadButton.disabled = false;
          },
          // Set margins to center the content nicely on the A4 page.
          margin: [15, 15, 15, 15],
          // A good option to prevent content from being cut off weirdly.
          autoPaging: "text",
          // Set the content width to match the A4 page width minus margins.
          // A4 width is 210mm. 210 - (15 * 2) = 180.
          width: 180,
          // Tell html2canvas to render the element at its actual browser width for best quality.
          windowWidth: ticketElement.offsetWidth
        };
        doc2.html(ticketElement, options);
      };
      this.downloadHandler = downloadTicket;
      downloadButton.addEventListener("click", this.downloadHandler);
    },
    // It's best practice to clean up event listeners when the element is removed.
    destroyed() {
      const downloadButton = document.getElementById("print_small_ticket");
      if (downloadButton && this.downloadHandler) {
        downloadButton.removeEventListener("click", this.downloadHandler);
      }
    }
  };
  Hooks2.BarGraph = {
    ...
    initChart() {
      const ticketTypes = JSON.parse(this.el.dataset.ticketTypes || "[]");
      const ticketCounts = JSON.parse(this.el.dataset.ticketCounts || "[]");
      console.log(ticketTypes, ticketCounts);
      const canvasId = this.el.querySelector("canvas").id;
      const canvas = document.getElementById(canvasId);
      if (this.chart) {
        this.chart.destroy();
      }
      let backgroundColor = "#9681B1";
      let label = "Tickets Sold by Type";
      if (canvasId.includes("payment-channels")) {
        backgroundColor = "#10B981";
        label = "Revenue by Payment Channel";
      }
    ...
    // Handle incoming data from LiveView push_event
    handleEvent(event, payload) {
      if (event === "update_ticket_chart") {
        this.el.dataset.ticketTypes = JSON.stringify(payload.types);
        this.el.dataset.ticketCounts = JSON.stringify(payload.counts);
        this.initChart();
      }
    }
  };
  Hooks2.LineGraph = {
    ...
    initChart() {
      ...
      const counts = JSON.parse(this.el.dataset.counts || "[]");
      const period = this.el.dataset.period || "daily";
      console.log(`Chart (${period}):`, dates, counts);
      ...
      let gradient, borderColor, label, tooltipPrefix;
      if (period === "revenue" || canvasId.includes("revenue")) {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                label: function(context) {
                  if (period === "revenue" || canvasId.includes("revenue")) {
                    return `Revenue: ${tooltipPrefix}${context.raw.toLocaleString()}`;
                  }
                  return `Sales: ${context.raw} tickets`;
                }
              }
            }
          },
          scales: {
            y: {
              ...
              ticks: {
                precision: 0,
                // Only show integers for sales charts
                font: {
                  size: 12
                },
                callback: function(value) {
    ...
    // Handle incoming data from LiveView push_event
    handleEvent(event, payload) {
      if (event === "update_sales_trend") {
        this.el.dataset.dates = JSON.stringify(payload.dates);
        this.el.dataset.counts = JSON.stringify(payload.counts);
        this.el.dataset.period = payload.period;
        this.initChart();
      }
      if (event === "update_revenue_trend") {
        this.el.dataset.dates = JSON.stringify(payload.dates);
        this.el.dataset.counts = JSON.stringify(payload.revenues);
        this.el.dataset.period = "revenue";
        this.initChart();
      }
    }
  };
  Hooks2.QrCode = {
    mounted() {
      ...
      function makeCode() {
        ...
        qrcode.makeCode("https://asakekenya.com/tickets/" + elText.value);
      }
      makeCode();
    },
    updated() {
